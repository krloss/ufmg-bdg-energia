...
function showAddress(val) {
    geocoder.geocode({'address':decodeURI(val)}, function(results,status) {
        ...
        else alert("Não foi possível encontrar um local aproximado com estes termos.");
    });
}
...
function geocode(position,pan) {
    ...
    geocoder.geocode({latLng:position},function(responses) {
        ...
        else html += 'Não foi possível determinar um endereço válido para essa localização';
...
function defaultLocation() {
    showMap(-19.922822,-43.945162); // praÃ§a raul soares
}
